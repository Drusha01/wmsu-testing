...
class StudentcetGrad extends Component

{
    ...
    public function mount(Request $request){
        ...
        $this->title = 'application-cet-graduate';
        ...
        $this->t_a_original_senior_high_school_card_id = rand(0,1000000);
    ...
    {
        return view('livewire.student.student-application.cet.studentgrad',[
    ...
    public function submit_application(Request $request){
        ...
        if($this->t_a_original_senior_high_school_card && file_exists(storage_path().'/app/livewire-tmp/'.$this->t_a_original_senior_high_school_card->getfilename())){
            $file_extension =$this->t_a_original_senior_high_school_card->getClientOriginalExtension();
            $tmp_name = 'livewire-tmp/'.$this->t_a_original_senior_high_school_card->getfilename();
            $size = Storage::size($tmp_name);
            $mime = Storage::mimeType($tmp_name);
            $max_image_size = 20 * 1024*1024; // 5 mb
            $file_extensions = array('image/jpeg','image/png','image/jpg');
            ...
            if($size<= $max_image_size){
                ...
                if($valid_extension){
                    $storage_file_path = storage_path().'/app/public/application-requirements/t_a_original_senior_high_school_card/';
                    
                    // move
                    $new_file_name = md5($tmp_name).'.'.$file_extension;
                    while(DB::table('test_applications')
                    ->where(['t_a_original_senior_high_school_card'=> $new_file_name])
                    ->first()){
                        $new_file_name = md5($tmp_name.rand(1,10000000)).'.'.$file_extension;
                    }
                    if(Storage::move($tmp_name, 'public/application-requirements/t_a_original_senior_high_school_card/'.$new_file_name)){
    
                        $this->t_a_original_senior_high_school_card_name = $new_file_name;
                        // resize thumb nail
                        // resize 500x500 px
                        $this->t_a_original_senior_high_school_card = null;
        ...
        if(DB::table('test_applications')->insert([
            ...
            't_a_original_senior_high_school_card' => $this->t_a_original_senior_high_school_card_name,]
        )){
            $this->dispatchBrowserEvent('swal:redirect',[
                'position'          									=> 'center',
                'icon'              									=> 'success',
