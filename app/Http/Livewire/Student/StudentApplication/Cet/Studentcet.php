...
class Studentcet extends Component
{
    ...
    public function submit_application(Request $request){
        ...
        if($this->t_a_school_principal_certification && file_exists(storage_path().'/app/livewire-tmp/'.$this->t_a_school_principal_certification->getfilename())){
            $file_extension =$this->t_a_school_principal_certification->getClientOriginalExtension();
            $tmp_name = 'livewire-tmp/'.$this->t_a_school_principal_certification->getfilename();
            $size = Storage::size($tmp_name);
            $mime = Storage::mimeType($tmp_name);
            $max_image_size = 20 * 1024*1024; // 5 mb
            $file_extensions = array('image/jpeg','image/png','image/jpg');
            ...
            if($size<= $max_image_size){
                ...
                if($valid_extension){
                    $storage_file_path = storage_path().'/app/public/application-requirements/t_a_school_principal_certification/';
                    
                    // move
                    $new_file_name = md5($tmp_name).'.'.$file_extension;
                    while(DB::table('test_applications')
                    ->where(['t_a_school_principal_certification'=> $new_file_name])
                    ->first()){
                        $new_file_name = md5($tmp_name.rand(1,10000000)).'.'.$file_extension;
                    }
                    if(Storage::move($tmp_name, 'public/application-requirements/t_a_school_principal_certification/'.$new_file_name)){
    
                        $this->t_a_school_principal_certification_name = $new_file_name;
                        // resize thumb nail
                        // resize 500x500 px
                        $this->t_a_school_principal_certification = null;
